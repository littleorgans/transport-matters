...
- **Slice 4 — next, gated**: `.agent-runtimes` registry seam + desktop request plumbing — thread a resolved `runtime_template` ref through the request so template mode is actually launchable (today nothing constructs a `RuntimeTemplateRef`). Targets the post-B6 `/v1` `CreateRunRequest` shape; B6 step 2 #123 + continuation #127 both merged → **fully unblocked**. **Carrier gotcha:** runtime-home planning's `launch_fields` WIN the carrier merge — route `runtime_template` through the planning path, not raw API `launch_fields` (see cm). The registry resolves a dual-target template (one dir holds both Claude and Codex config) and strips generator-internal `runtime.toml`; cross-client files are harmless (Claude ignores Codex-domain files and vice versa).
