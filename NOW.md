...
- **Canvas keyboard zoom/pan migration — revisit.** `+`/`-`/`=` zoom and `Alt`+Arrow pan (`engine/react/useCanvasViewport.ts`) were deliberately deferred out of the keybinding registry (#146-149) and the launcher action/nav redesign (#151-153). With the registry/engine + declarative action model now shipped, folding them in is a one-entry-per-command change — and the moment to fix the known `Alt`+Arrow vs browser Back/Forward collision (a11y). Design: `~/.mdx/projects/transport-matters-launcher-design--authoritative.md` (deferred-items note).
