...
from manicure.cli import BindFailure, main
...
def _patch_allocate_pairs(
    ...
    ``manicure.cli.allocate_port_pair``; the retry-time re-allocation in
    ``cli.runner._handle_bind_failure`` resolves it through
    ``manicure.cli.runner.allocate_port_pair``. Patching one site only
    ...
    monkeypatch.setattr("manicure.cli.allocate_port_pair", _alloc)
    monkeypatch.setattr("manicure.cli.runner.allocate_port_pair", _alloc)
    return drawn
...
def test_start_dynamic_ports_appear_in_print_command(
    ...
    monkeypatch.setattr(
        "manicure.cli.allocate_port_pair", lambda *_a, **_k: (54321, 54322)
    )
...
def test_start_explicit_proxy_port_overrides_allocation(
    ...
    monkeypatch.setattr("manicure.cli.allocate_port_pair", sentinel)
...
def test_start_port_allocation_error_surfaces_actionable_message(
    ...
    from manicure.cli import PortAllocationError
    ...
    monkeypatch.setattr("manicure.cli.allocate_port_pair", _raise)
...
def test_start_injects_system_prompt_by_default(
    ...
    proxy + inspector URLs so the model knows it is inside manicure."""
    monkeypatch.setattr(
        "manicure.cli.shutil.which",
        _which_by_name({"mitmdump": "/bin/mitmdump", "claude": "/bin/claude"}),
    )
