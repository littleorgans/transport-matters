...
def test_run_children_spawns_claude_with_pty(
    ...
    with pytest.raises(typer.Exit):
        _run_children(
            ...
            claude_env={"ANTHROPIC_BASE_URL": "http://localhost:8787"},
            claude_cwd=tmp_path,
            proxy_port=8787,
            web_port=8788,
        )
...
def test_handle_bind_failure_pinned_web_port_fails_fast(
    ...
    spy = MagicMock()
    monkeypatch.setattr("manicure.cli.runner.allocate_port_pair", spy)
...
def test_handle_bind_failure_reallocates_only_named_unpinned_slot(
    ...
    monkeypatch.setattr(
        "manicure.cli.runner.allocate_port_pair",
        lambda: (60001, 60002),
    )
    exc = _make_failure(
...
def test_handle_bind_failure_propagates_allocator_error(
    ...
    from manicure.cli.ports import PortAllocationError

    def _raise() -> tuple[int, int]:
        raise PortAllocationError("kernel said no")

    monkeypatch.setattr("manicure.cli.runner.allocate_port_pair", _raise)
