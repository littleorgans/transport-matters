"""Shared fixtures for ``manicure.cli`` tests.
...
def spy_run_children(monkeypatch: pytest.MonkeyPatch) -> MagicMock:
    ...
    The retry harness in :func:`manicure.cli.runner._run_with_retry`
    ...
    ``manicure.cli._run_children`` is unaffected; tests that read
    ``call_args`` against the returned spy work either way.
    """
    spy = MagicMock()
    monkeypatch.setattr("manicure.cli.runner._run_children", spy)
    return spy
