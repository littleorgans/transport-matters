...
Split out from :mod:`manicure.cli` so the package entry point stays
...
Tests that previously patched ``manicure.cli.shutil.which`` /
``manicure.cli._port_in_use`` continue to work — those re-exports are
