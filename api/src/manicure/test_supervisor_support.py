"""Shared test support for `manicure.supervisor`."""
...
if TYPE_CHECKING:
    ...
    from manicure.supervisor import ManagedProcess
...
def pty_stubs(monkeypatch: pytest.MonkeyPatch) -> dict[str, Any]:
    ...
    monkeypatch.setattr("manicure.supervisor.threading.Thread", StubThread)
    monkeypatch.setattr("manicure.supervisor.sys.stdin", FakeStdio(fileno=0))
    monkeypatch.setattr("manicure.supervisor.sys.stdout", FakeStdio(fileno=1))
