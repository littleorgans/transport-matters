...
class SessionDao:
    ...
    def count_dead_letters_by_run(self, run_ids: Sequence[str]) -> dict[str, int]:
        if not run_ids:
            return {}
        rows = self._conn.execute(
            COUNT_DEAD_LETTERS_BY_RUN_SQL, {"run_ids": list(run_ids)}
        ).fetchall()
        return {str(row["run_id"]): int(row["dead_letter_count"]) for row in rows}

    def count_dead_letters_by_session(self, session_ids: Sequence[str]) -> dict[str, int]:
        if not session_ids:
            return {}
        rows = self._conn.execute(
