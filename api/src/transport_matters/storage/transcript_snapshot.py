...
Tier-1 holds only the wire; the transcript lives only in the harness native file
(``~/.claude/projects/...``, ``~/.codex/sessions/...``), which the harness or the user can GC. When
...
§9.2 tailer reads them, so a future rebuild replays TM's OWNED bytes regardless of harness retention.
...
Idempotence (the load-bearing property): the snapshot is a byte-faithful copy of the harness file
consumed prefix, so its on-disk size **is** the length of the prefix already owned. A re-tail (a
fresh process re-reads the harness file from offset 0) re-tees a range we already hold; we append only
...
def make_transcript_snapshot_writer(storage_root: Path) -> Callable[[str, int, bytes], None]:
    ...
    bytes for *session_id*, where *consumed* mirrors the harness byte range
    ...
    def snapshot(session_id: str, start_offset: int, consumed: bytes) -> None:
        ...
        # The snapshot is a byte-faithful prefix of the harness file, so snap_size is the prefix length
        # already owned. The incoming bytes cover harness range [start_offset, start_offset+len).
