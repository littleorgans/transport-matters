Synced mirror of `~/.mdx/projects/transport-matters-codex-seam-audit.md`.

# Transport Matters Codex Seam Audit

## Summary

Transport Matters already has three useful provider seams:
...
| --- | --- | --- |
| Launch and child process ownership | Extensible, with blocker | `ProcessSupervisor` is generic, but `transport-matters claude` is Claude specific in CLI copy, binary resolution, system prompt injection, reverse proxy setup, and `ANTHROPIC_BASE_URL` export. References: `api/src/transport_matters/cli/__init__.py:153-200`, `api/src/transport_matters/cli/__init__.py:452-497`, `api/src/transport_matters/cli/runner.py:248-263`, `api/src/transport_matters/cli/prompt.py:1-63`, `api/src/transport_matters/supervisor.py:148-263`. |
| Adapter registry and IR | Real seam, but incomplete | `ProviderAdapter` and `get_adapter()` are real abstractions, and `InternalRequest` already preserves opaque provider fields. The contract is too narrow for WebSocket sessions because it only covers raw request and response bytes. References: `api/src/transport_matters/adapters/base.py:17-40`, `api/src/transport_matters/adapters/__init__.py:1-33`, `api/src/transport_matters/ir.py:44-64`, `api/src/transport_matters/ir.py:79-145`. |
| Addon transport entry | Anthropic specific blocker | `the addon.request()` exits unless the path starts with `/v1/messages`, before adapter selection even runs. ChatGPT Codex traffic will never hit the pipeline. References: `api/src/transport_matters/addon.py:439-446`, `api/src/transport_matters/adapters/anthropic.py:55-56`. |
| Breakpoint state and recount | Extensible, with blocker | Paused flow state is generic around IR, but recount stores Anthropic auth headers and reserializes through `AnthropicAdapter`. References: `api/src/transport_matters/breakpoint.py:24-42`, `api/src/transport_matters/api/v1/breakpoint_routes.py:14-15`, `api/src/transport_matters/api/v1/breakpoint_routes.py:36-55`. |
| Token counting | Anthropic specific blocker | Counting logic is explicitly built around Anthropic auth headers and `/v1/messages/count_tokens`. Exchange lazy recount uses `AnthropicAdapter()` directly. References: `api/src/transport_matters/counting.py:1-56`, `api/src/transport_matters/counting.py:105-130`, `api/src/transport_matters/api/v1/exchanges.py:12-18`, `api/src/transport_matters/api/v1/exchanges.py:107-205`. |
| Persistence and exchange API | Extensible, with blocker | Summary index fields are provider neutral, but stored artifacts only cover request and response blobs plus IR snapshots. There is no place for upgrade headers, initial request frame, or streamed upstream events. References: `api/src/transport_matters/storage/base.py:63-90`, `api/src/transport_matters/storage/disk.py:207-243`, `api/src/transport_matters/api/v1/exchanges.py:44-86`. |
| Override pipeline | Reusable | The override system already supports generic `sampling_set` and dotted `provider_extras_set` writes. That is enough for Codex specific request fields to flow through the existing rule pipeline once the request is normalized into IR. References: `api/src/transport_matters/overrides.py:30-73`, `api/src/transport_matters/overrides.py:372-482`, `api/src/transport_matters/overrides.py:676-870`. |
...
- `ProcessSupervisor.spawn()` already supports foreground PTY children and background log owned daemons. That is enough for a Codex launcher once argv and env construction move behind a client specific builder. `api/src/transport_matters/supervisor.py:148-263`
- The IR is intentionally opaque in the right places. `provider_extras`, `provider_data`, and `UnknownBlock` give a Codex adapter somewhere to keep frame specific fields without widening the core models first. `api/src/transport_matters/ir.py:44-64`, `api/src/transport_matters/ir.py:79-145`, `api/src/transport_matters/ir.py:157-168`
- The override engine does not care which provider owns a field. `provider_extras_set` already supports dotted nested writes with safe deletion and path validation. `api/src/transport_matters/overrides.py:388-482`
...
References: `api/src/transport_matters/adapters/base.py:17-40`, `api/src/transport_matters/addon.py:439-493`
...
References: `api/src/transport_matters/cli/__init__.py:153-200`, `api/src/transport_matters/cli/__init__.py:452-497`, `api/src/transport_matters/cli/runner.py:248-263`, `api/src/transport_matters/cli/prompt.py:1-63`
...
References: `api/src/transport_matters/counting.py:26-56`, `api/src/transport_matters/counting.py:105-130`, `api/src/transport_matters/api/v1/breakpoint_routes.py:36-55`, `api/src/transport_matters/api/v1/exchanges.py:107-205`, `api/src/transport_matters/breakpoint.py:34-38`
...
References: `api/src/transport_matters/storage/base.py:81-90`, `api/src/transport_matters/storage/disk.py:207-243`
...
- CLI help, banner text, and injected prompt copy are Claude centric. This should be updated as part of the Codex launcher work, not before. `api/src/transport_matters/cli/help.py:24-110`, `api/src/transport_matters/cli/prompt.py:1-63`
...
- Do not try to bolt Codex onto the existing `/v1/messages` branch in `the addon.request()`. Start by making transport dispatch happen before Anthropic specific path gating.
