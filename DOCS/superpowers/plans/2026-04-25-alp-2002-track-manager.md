...
**Architecture:** Add a pure `manicure.track_manager` state machine keyed by `run_id`. Recorders classify the request before persistence, then observe the response to register or close child tracks. Storage persists track fields on `IndexEntry`; old entries load with `track_id = run_id` when `run_id` exists.
...
- Files to create or modify:
  - Create `api/src/manicure/track_manager.py` for the pure state machine.
  - Create `api/src/manicure/test_track_manager.py` for reference trace and synthetic unit tests.
  - Modify `api/src/manicure/storage/base.py` to add persisted track fields.
  - Modify `api/src/manicure/exchange_recorder.py` and `api/src/manicure/codex/exchange.py` to classify and persist track metadata.
  - Modify `api/src/manicure/codex/exchange_derivation.py` only if provisional rewrite events need track metadata in SSE payloads.
...
- [ ] Add `api/src/manicure/test_track_manager.py` with helpers for synthetic IR and reference trace loaders.
...
- [ ] Run `cd api && uv run pytest src/manicure/test_track_manager.py -v`; expected failure is missing `manicure.track_manager`.

### Task 2: Minimal pure state machine

- [ ] Implement `api/src/manicure/track_manager.py` with no disk or network I/O.
- [ ] Support Claude `Agent`, Codex `spawn_agent`, Codex `wait_agent`, Codex `agent_kill`, failed Codex spawn results, fan out, and nesting.
- [ ] Run `cd api && uv run pytest src/manicure/test_track_manager.py -v`; expected pass.
...
- [ ] Run `cd api && uv run pytest src/manicure/storage/test_disk.py -v`.
...
- [ ] Run `cd api && uv run ruff format src/ && uv run ruff check src/ && uv run mypy src/ && uv run pytest src/manicure/test_track_manager.py src/manicure/storage/test_disk.py`.
